...
export class ChatsActions {
	...
	constructor(private store: Store<AppState>, private api: ApiService, private events: Events) {
			...
			.then(response => {
				if (!id && response) {
					...
					if (
						...
						response.result.length < CHATS_LIMIT_PER_REQUEST
					) {
	...
	postMessage = ({ id, payload }: models.PostMessage) =>
		this.api
			.call({
				types: {
					request: types.CHATS_REQUEST,
	...
	onNewMessage = ({ id, item }) => {
		const shouldAdd = !(<MessageModel[]>this.state.entities.chats[id].messages).includes(
			item._id
		);
